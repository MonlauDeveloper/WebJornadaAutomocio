...
<head>
    ...
    <title>Perfil de {{ $student->name }} | Monlau Automoción</title>
...
<div class="container mx-auto px-4 max-w-5xl">
    ...
    <div class="bg-white p-8 rounded-3xl shadow-xl border border-gray-100 text-center mb-8">
        ...
        <h1 class="text-4xl font-extrabold text-slate-800 mt-6">{{ $student->name }} {{ $student->surname1 }} {{ $student->surname2 }}</h1>
        
        <div class="flex flex-wrap justify-center gap-4 mt-4">
            <span class="bg-blue-50 text-blue-700 px-4 py-1 rounded-full text-sm font-bold">
                {{ $student->specialization->specialization ?? 'Especialización no definida' }}
            </span>
            @if($student->team)
            <span class="bg-slate-100 text-slate-600 px-4 py-1 rounded-full text-sm font-bold">
                Equipo: {{ $student->team->teamName }}
            </span>
            @endif
        </div>
    </div>
    ...
    <div class="space-y-6">
        ...
        <div class="bg-white p-8 rounded-3xl shadow-lg border border-gray-50">
            ...
            <p class="text-slate-600 leading-relaxed text-lg italic">
                "{{ $student->introduction ?? 'Sin descripción profesional disponible.' }}"
            </p>
        </div>
                ...
                <div class="space-y-4">
                    @forelse($student->workExperiences as $experience)
                        <div class="flex items-start text-slate-700">
                            ...
                            <p class="font-medium">{{ $experience->work_experience }}</p>
                        </div>
                    @empty
            ...
            <div class="bg-white p-8 rounded-3xl shadow-lg border border-gray-50">
                ...
                <div class="space-y-4">
                    @forelse($student->educations as $education)
                        <div class="flex items-start text-slate-700">
                            <span class="w-2 h-2 bg-blue-400 rounded-full mr-3 mt-2"></span>
                            <p class="font-medium">{{ $education->education }}</p>
                        </div>
                    @empty
                        <p class="text-slate-400 italic">No especificada</p>
                    @endforelse
