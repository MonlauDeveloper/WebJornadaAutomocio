...
@section('content')
    ...
    <div class="container mx-auto p-4 sm:p-6">
        <div class="bg-white p-4 sm:p-6 rounded-lg shadow-lg max-w-7xl mx-auto">
            <h1 class="text-3xl sm:text-4xl font-bold text-blue-600 text-center">Mi Proyecto</h1>
            ...
            @if($projectIncomplete)
                <div class="text-center text-red-500 mt-4 mb-4">
                    ...
                    <p class="mt-2 text-sm sm:text-base">Tu proyecto está incompleto. Por favor, completa todos los campos.</p>
                </div>
            @endif

            <!-- Mensajes de feedback -->
            @if(session('success'))
                <div id="success-message" class="js-feedback-message bg-green-100 border border-green-400 text-green-700 px-4 py-3 rounded relative mb-6 text-center font-semibold animate-fade-in text-sm sm:text-base">
                    <i class="fas fa-check-circle mr-2"></i>{{ session('success') }}
                </div>
            @endif

            @if(session('error'))
                <div id="error-message" class="js-feedback-message bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded relative mb-6 text-center font-semibold animate-fade-in text-sm sm:text-base">
                    <i class="fas fa-times-circle mr-2"></i>{{ session('error') }}
                </div>
            @endif

            <form action="{{ route('projects.update', $project->idProject) }}" method="POST" enctype="multipart/form-data"
                class="bg-white p-2 sm:p-6 rounded-lg shadow-lg">
                @csrf
                @method('PUT')
                ...
                <div class="mb-6">
                    <label class="block text-gray-700 font-semibold mb-4 sm:mb-7">
                        Tipos de Proyecto
                    ...
                    <div class="border border-gray-300 rounded-lg p-4 bg-white max-h-60 overflow-y-auto shadow-inner">
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-1">
                            @foreach($projectTypes as $type)
                                <label class="flex items-center space-x-3 p-2 hover:bg-gray-50 rounded cursor-pointer transition text-sm sm:text-base">
                ...
                <div class="mb-4">
                    <label for="photoName" class="block text-gray-700 font-semibold mb-2">Foto del Proyecto (Portada Orla)</label>
                    <div class="flex flex-col sm:flex-row items-stretch sm:items-center gap-4">
                        <input type="file" id="photoName" name="photoName"
                            class="p-2 sm:p-3 w-full rounded-lg border border-gray-300 focus:outline-none focus:ring-2 focus:ring-blue-500 text-sm">

                        @if ($project->photoName && $project->photoName !== 'por_defecto/proyecto_default.png')
                            <button type="button"
                                onclick="if(confirm('¿Restaurar imagen por defecto?')) document.getElementById('delete-main-photo-form').submit();"
                                class="bg-blue-50 text-blue-600 hover:bg-blue-600 hover:text-white p-3 rounded-lg border border-blue-200 transition flex items-center justify-center shrink-0"
                    ...
                    @if ($project->photoName)
                        <div class="mt-3 flex items-center gap-4 p-2 border rounded-lg bg-gray-50 overflow-hidden">
                            @php
                            ...
                            @endphp
                            <img src="{{ $photoUrl }}" alt="Foto actual" class="h-12 w-12 sm:h-16 sm:w-16 object-contain bg-white rounded border flex-shrink-0">
                            <div class="flex flex-col min-w-0">
                                <span class="text-[10px] font-bold text-blue-600 uppercase">Archivo actual:</span>
                                <span class="text-[10px] text-gray-500 truncate">{{ basename($project->photoName) }}</span>
                            </div>
                        </div>
                    @endif
                </div>
                    ...
                    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                        <div class="lg:col-span-1 bg-white p-5 rounded-lg border border-gray-300 shadow-sm h-fit">
                            <div class="flex justify-between items-center mb-4 border-b pb-2">
                                ...
                                <span class="text-[10px] font-bold {{ $totalFotos >= 6 ? 'text-red-500' : 'text-green-600' }}">
                                    {{ $totalFotos }}/6
                                </span>
                            </div>

                            @if($totalFotos >= 6)
                                <div class="bg-red-50 border border-red-200 text-red-700 p-2 rounded text-[10px] mb-4 text-center">
                                    <strong>Límite alcanzado.</strong> Borra una para subir otra.
                                </div>
                            @endif
                            ...
                            <div class="space-y-4 {{ $totalFotos >= 6 ? 'opacity-40 pointer-events-none' : '' }}">
                                <div>
                                    ...
                                    {{ $totalFotos >= 6 ? 'disabled' : '' }}>
                                </div>

                                <div class="grid grid-cols-1 gap-4 items-end">
                                    <div>
                                        <label class="block text-xs font-bold text-gray-500 uppercase mb-2">2. Sección</label>
                                        <select name="new_image_fase" id="image_fase_selector" class="block w-full rounded border-gray-300 text-sm shadow-sm focus:ring-blue-500">
                                    ...
                                    <div class="w-full">
                                        <button type="submit" name="action" value="upload_image" class="w-full bg-green-600 hover:bg-green-700 text-white font-bold py-2.5 rounded text-xs transition shadow-sm uppercase">
                                            Subir esta Imagen
                                        </button>
                                    </div>
                                </div>
                                ...
                                <div id="save-warning" class="hidden mt-3 p-2 bg-amber-50 border border-amber-200 rounded text-center">
                                    <p class="text-[9px] text-amber-700 font-bold uppercase flex items-center justify-center">
                                        ¡Archivo listo! Pulsa "Subir Imagen".
                                    </p>
                                </div>
                            </div>
                        </div>

                        <div class="lg:col-span-2 bg-gray-50 p-3 sm:p-5 rounded-lg border border-gray-300">
                            <div class="flex flex-col sm:flex-row items-center justify-between mb-4 gap-3">
                                ...
                                <button type="button" onclick="openPreviewModal()"
                                    class="w-full sm:w-auto bg-blue-500 hover:bg-blue-700 text-white text-[10px] font-bold py-2 px-4 rounded shadow-sm transition flex items-center justify-center gap-2 uppercase">
                            ...
                            <div class="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                                @foreach($project->images as $img)
                                    <div class="relative bg-white p-2 rounded border border-gray-200 shadow-sm flex flex-col min-w-0">
                                        <div class="rounded overflow-hidden aspect-video bg-gray-100 mb-2">
                                            <img src="{{ asset('storage/project_steps/' . $img->file_path) }}" class="w-full h-full object-cover">
                                        </div>
                                        ...
                                        <span class="block text-[7px] font-bold text-center py-1 rounded border {{ $badgeColor }} uppercase mb-2 truncate">
                                            {{ $label }}
                                        </span>

                                        @if($img->fase !== 'header')
                                            <div class="mt-auto">
                                                <textarea name="image_descriptions[{{ $img->id }}]" rows="2" maxlength="300"
                                                    class="w-full text-[10px] border-gray-200 rounded p-1 mb-1 focus:ring-1 focus:ring-blue-400"
                                                    placeholder="Descripción...">{{ $img->description }}</textarea>
                                                <button type="button" onclick="saveImageText(this, {{ $img->id }})"
                                                    class="w-full bg-blue-500 hover:bg-blue-600 text-white text-[8px] font-bold py-1.5 rounded transition uppercase">
                                                    Guardar
                                                </button>
                                            </div>
                                        @endif
                                        ...
                                        <button type="button"
                                            ...
                                            class="absolute -top-1.5 -right-1.5 bg-red-500 text-white rounded-full h-4 w-4 flex items-center justify-center border-2 border-white shadow hover:bg-red-700 transition-colors">
                ...
                <div class="mb-4 p-4 border rounded-lg bg-gray-50">
                    ...
                    <textarea name="conclusion" id="conclusion" rows="4"
                        class="w-full border-gray-300 rounded-lg shadow-sm focus:ring focus:ring-blue-200"
                        placeholder="Escribe aquí el resultado final..." maxlength="300">{{ old('conclusion', $project->conclusion) }}</textarea>
                </div>

                <div class="mt-8 pt-6 border-t">
                    <div class="flex flex-col sm:flex-row flex-wrap gap-3 justify-center sm:justify-start">
                        <button type="submit" class="bg-blue-600 hover:bg-blue-700 text-white font-bold py-3 px-6 rounded shadow w-full sm:w-auto">
                            Guardar Cambios
                        </button>

                        @if ($project->idProject)
                            <a href="{{ route('project.pdf', $project->idProject) }}"
                                class="bg-green-600 hover:bg-green-700 text-white font-bold py-3 px-6 rounded shadow text-center w-full sm:w-auto">
                                Ver Ficha Técnica
                            </a>
                        @endif

                        <a href="javascript:history.back()" class="bg-gray-600 hover:bg-gray-700 text-white font-bold py-3 px-6 rounded shadow text-center w-full sm:w-auto">
                            Volver
                        </a>

                        <button type="button" onclick="if(confirm('¿Eliminar proyecto?')) document.getElementById('delete-project-form').submit();"
                            class="bg-red-700 hover:bg-red-800 text-white font-bold py-3 px-6 rounded shadow w-full sm:w-auto">
                            Eliminar Proyecto
                        </button>
                    </div>
                </div>
            ...
            <div id="previewModal" class="fixed inset-0 z-50 hidden overflow-y-auto bg-black bg-opacity-80 flex items-center justify-center p-2 sm:p-4">
                <div class="relative bg-white w-full max-w-5xl rounded-lg shadow-2xl overflow-hidden animate-fade-in flex flex-col" style="max-height: 95vh;">

                    <div class="bg-gray-100 px-4 sm:px-6 py-4 flex justify-between items-center border-b">
                        <h2 class="text-sm sm:text-lg font-bold text-gray-700 uppercase">Vista Previa PDF</h2>
                    ...
                    <div class="p-2 sm:p-8 bg-gray-400 overflow-y-auto flex-grow shadow-inner">
                        <!-- El contenedor del PDF ahora se adapta a la pantalla pero mantiene el max-width de un folio -->
                        <div id="pdf-paper" class="bg-white mx-auto p-6 sm:p-12 text-gray-900 relative shadow-2xl"
                            style="width: 100%; max-width: 210mm; min-height: 297mm; font-family: 'Helvetica', Arial, sans-serif;">
                            ...
                            <div class="flex flex-col sm:flex-row justify-between items-start mb-10 border-b-2 border-gray-100 pb-6 relative z-10 gap-6">
                                <div class="w-full sm:w-2/3">
                                    <h1 id="modal-preview-title" class="text-2xl sm:text-3xl font-bold text-blue-800 uppercase italic leading-tight break-words">
                                        {{ $project->title }}
                                    </h1>
                                ...
                                <div class="w-full sm:w-1/3 flex justify-center sm:justify-end">
                                    ...
                                    <div class="border-4 border-white shadow-lg transform sm:rotate-2 overflow-hidden w-40 h-28 bg-gray-100 flex-shrink-0">
                                        @if($portada)
                                            <img src="{{ asset('storage/project_steps/' . $portada->file_path) }}" class="w-full h-full object-cover">
                                        @else
                                            <div class="w-full h-full flex items-center justify-center text-[8px] text-gray-400 italic text-center p-2">Sin portada</div>
                                        @endif
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="mb-8 border border-gray-100 p-4 rounded bg-blue-50 bg-opacity-30 relative z-10">
                                ...
                                <p id="modal-preview-abstract" class="text-sm text-justify leading-relaxed break-words italic">{{ $project->abstract }}</p>
                            </div>
                            ...
                            <div id="modal-preview-steps" class="space-y-8 relative z-10">
                                ...
                                @foreach($pasos as $index => $img)
                                    <div class="flex flex-col sm:flex-row gap-6 items-center {{ $index % 2 != 0 ? 'sm:flex-row-reverse' : '' }}">
                                        <div class="w-full sm:w-2/5 border p-1 rounded bg-white shadow-sm">
                                            <span class="text-[9px] font-bold text-blue-700 block border-b mb-1 uppercase tracking-tighter">Paso Técnico {{ $img->orden ?? ($index + 1) }}</span>
                                            <img src="{{ asset('storage/project_steps/' . $img->file_path) }}" class="w-full h-32 sm:h-36 object-contain bg-gray-50">
                                        </div>
                                        <div class="w-full sm:w-3/5">
                                            <p class="text-xs text-gray-600 text-justify italic step-desc-preview" data-id="{{ $img->id }}">
                                ...
                                <div class="mt-12 pt-8 border-t-2 border-dashed border-gray-100 relative z-10">
                                    <div class="flex flex-col sm:flex-row items-center gap-6">
                                        <div class="w-full sm:w-3/4">
                                            ...
                                            <p class="text-xs text-gray-500 italic step-desc-preview" data-id="{{ $final->id }}">
                                                {{ $final->description ?? 'El proyecto se ha completado según los objetivos previstos...' }}
                                            </p>
                                        </div>
                                        <div class="w-28 sm:w-1/4">
                                            <img src="{{ asset('storage/project_steps/' . $final->file_path) }}" class="w-full border-2 border-white shadow-md rounded-sm">
                                        </div>
                                    </div>
                                    <h3 class="text-green-700 font-bold uppercase text-[10px] tracking-widest mb-2 flex items-center gap-2 mt-6">
                                        <span class="w-2 h-2 bg-green-500 rounded-full"></span>
                                        Conclusión final
                                    </h3>
                                    <p id="modal-preview-conclusion" class="text-xs text-gray-500 italic break-words">
                                        {{ $project->conclusion ?? 'El proyecto se ha completado satisfactoriamente.' }}
                                    </p>
                                </div>
                            @endif
                        </div>
                    ...
                    <div class="bg-gray-100 px-6 py-3 text-right border-t text-[10px] text-gray-400 uppercase tracking-widest font-semibold italic">
                        Revisión dinámica de documento
                    </div>
                </div>
            </div>

            <form id="delete-project-form" action="{{ route('projects.destroy', $project->idProject) }}" method="POST" class="hidden">@csrf @method('DELETE')</form>
            <form id="delete-main-photo-form" action="{{ route('projects.photo.destroy', $project->idProject) }}" method="POST" class="hidden">@csrf @method('DELETE')</form>
            @foreach($project->images as $img)
                <form id="delete-img-{{ $img->id }}" action="{{ route('projects.image.destroy', $img->id) }}" method="POST" class="hidden">@csrf @method('DELETE')</form>
            @endforeach
        </div>
        ...
        function saveImageText(button, imageId) {
            ...
            fetch("{{ route('projects.update', $project->idProject) }}", {
                ...
                body: JSON.stringify({
                    ...
                    image_descriptions: { [imageId]: text },
                    _method: 'PUT'
                })
            })
            .then(response => {
