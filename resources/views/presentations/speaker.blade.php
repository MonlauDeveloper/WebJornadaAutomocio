...
<div class="container mx-auto p-6">
    <div class="bg-white p-6 rounded-lg shadow-lg max-w-4xl mx-auto">
        <h2 class="text-4xl font-bold text-blue-600 text-center">Ponentes de la Presentación: {{ $presentation->presentationName }}</h2>
        ...
        <ul class="mt-8">
            @foreach ($presentation->speakers as $speaker)
                <li class="bg-gray-200 p-4 rounded-lg shadow-md mb-4">
                    <p>{{ $speaker->name }} {{ $speaker->surname1 }} {{ $speaker->surname2 }}</p>
                    <p>{{ $speaker->description }}</p>

                    <div class="mt-4 flex space-x-4">
                        <!-- Enlace para editar -->
                        <a href="{{ route('presentations.editSpeaker', [$presentation->idPresentation, $speaker->idSpeaker]) }}" class="text-blue-500">Editar</a>

                        <!-- Formulario para eliminar ponente -->
                        <form action="{{ route('presentations.removeSpeaker', [$presentation->idPresentation, $speaker->idSpeaker]) }}" method="POST" onsubmit="return confirm('¿Estás seguro de eliminar este ponente?');">
                            @csrf
                            @method('DELETE')
        ...
        <form action="{{ route('presentations.addSpeaker', $presentation->idPresentation) }}" method="POST">
            @csrf
            <div class="mt-4">
